    ...
    <div class="nav-wrapper container-fluid">
      ...
      @else
        <ul class="right hide-on-med-and-down">
          <li><a class="btn" data-toggle="modal" style="margin-top:1em;margin-bottom:1em;" data-target="#order_popup_dialog"><i class="material-icons left">payment</i> Order Now</a></li>
...
$(document).ready(function(){
  ...
  $('#order_details').append('<li>'+sessionStorage.getItem('full_name') + '</li>');
  $('#order_details').append('<li>'+sessionStorage.getItem('phone_number') + '</li>');
  $('#order_details').append('<li>'+ sessionStorage.getItem('address')+ '</li>');
  $('#order_details').append('<li> Order For: '+sessionStorage.getItem('num_people') + '</li>');
  var choices=JSON.parse(sessionStorage.getItem('selected_choices'));
  $.each(choices,function(index,choice){
    $.get('get_menus.php',{choice:choice},function(data,status){
      if(status=='success'){
        var results=JSON.parse(data);
        $.each(results,function(index,obj){
          if(jQuery.type(obj) !== "undefined"||obj.name==="undefined"){
            $('#menu_items').append('<div class="col m4"><div class="card "><div class="card-content"><p style="float:left;color:black;"><b>Item Number: ' + obj.item_number + "</b></br>Category:"+obj.category_name
                +'<br/> Item Name: '+ obj.name+'</br><select id="item_sizes"><option value="" disabled selected>Choose your size</option><option value="Sandwich">Sandwich</option><option value="Medium Sub">Medium Sub</option><option value="Large Sub">Large Sub</option><option value="Wrap">Wrap</option></select></p><br/></div><br/><input type="number" placeholder="Quantity" style="width:6em;margin-left:2em;"/></div>');
            }
              $('select').material_select();
          });
      }
    ...
    .fail(function() {
      ...
      $.notify('An error occured loading menu items, pelase try again',{
        type:"danger",
        align:"center",
...
$(this).material_select();
  ...
  <div class="container-fluid" style="margin-top:8em">
    <div class="row">
      <div class="col-md-3">
        {{-- <header class="center"><h5>Order details</h5></header> --}}
      ...
      <div class="col-md-9">
        <header class="center"><h5>Please select order choices</h5></header>
